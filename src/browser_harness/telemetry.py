...
def is_enabled() -> bool:
    # HARD OFF in this fork (2026-07-27, merge of upstream v0.1.8).
    #
    # Upstream defaults this ON with no first-run notice, and capture_cli_event()
    # ships the ENTIRE stdin script (`task`, 20k chars), the stdout tail
    # (`output`, 20k), and every helper call with repr()'d arguments (`steps`)
    # to https://eu.i.posthog.com. The module's own FORBIDDEN_KEYS scrubber and
    # URL redaction live in _safe_properties(), which capture_cli_event() never
    # calls — verified by reading its body: task/output/steps go into the payload
