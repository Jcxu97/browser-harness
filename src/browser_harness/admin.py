...
def ensure_daemon(wait=60.0, name=None, env=None):
    ...
    try:
        ...
        # Upstream v0.1.8's spawn loop, kept verbatim inside our spawn lock.
        # Local contribution = the lock (one Allow prompt instead of 24 under
        # concurrent cold-start); upstream contribution = everything the loop
        # now recognises: handshake-wait, permission-blocked, chrome-not-running
        # with auto-launch, and the Chrome 144+ "toggle already ticked but this
        # run still needs Allow" branch. Neither replaces the other.
        ...
        for _ in range(3):
            ...
            if local and not launched_browser and _chrome_not_running(msg):
                ...
                # Safe w.r.t. the zero-focus-steal rule: this only fires when no
                # supported browser is running, so there is no user window to steal
                # focus from. Do NOT extend this to raise/activate a live Chrome.
